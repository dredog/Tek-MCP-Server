...
# v1.1.1 upgrade components
try:
    from tek_pta_v111_upgrade import (
        SplashScreen, SuiteSelector, ChannelSourceSelector,
        RunQueueManager, HelpViewer, FavoritesManager,
    )
    V111_AVAILABLE = True
except ImportError:
...
class TestSuite:
    ...
    created_time: str = ""  # Created timestamp (ISO format or display string)
            ...
            try:
                ...
                if hasattr(module, 'register_suites'):
                    ...
                    try:
                        ...
                        cre_time = datetime.datetime.fromtimestamp(stat.st_ctime)
                        cre_time_str = cre_time.strftime("%Y-%m-%d %H:%M")
                    except:
                        now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
                        mod_time_str = now_str
                        cre_time_str = now_str
...
class InstrumentManager:
    ...
    def scope_read_file(self, scope_path: str, timeout_ms: int = 10000) -> bytes:
        """Read a binary file from the scope with timeout protection.
        
        Uses FILESystem:READFile with a dedicated timeout to prevent
        blocking the engine thread if the transfer stalls.
        
        Args:
            scope_path: Path on scope filesystem (e.g. "C:/Temp/test.png")
            timeout_ms: Timeout for binary read in milliseconds (default 10s)
        
        Returns:
            Raw bytes of the file, or empty bytes on failure
        
        Raises:
            Exception on timeout or VISA error
        """
        if not self.scope:
            return b""
        ...
        try:
            self._log_scpi("cmd", f'SCOPE << FILESystem:READFile "{scope_path}"')
            self.scope.write(f'FILESystem:READFile "{scope_path}"')
            data = self.scope.read_raw()
            self._log_scpi("response", f"SCOPE >> [{len(data)} bytes]")
            return data
        except Exception as e:
            self._log_scpi("response", f"SCOPE >> FILE READ ERROR: {e}")
            # Clear any partial data from the VISA buffer
            try:
                self.scope.write("*CLS")
            except Exception:
                pass
        ...
        try:
            ...
            for scope_path in scope_paths:
                try:
                    ...
                    data = self.inst.scope_read_file(scope_path)
            ...
            if not data:
                self._log("Screenshot skipped (transfer failed)")
                return ""
...
class LEDCurrentTestEngine:
    ...
    def __init__(self, inst: InstrumentManager):
        ...
        # Channel settings (set from UI before run_sequence)
        self.termination = "1M"    # "50" or "1M"
        self.bandwidth = "20E6"    # "FULL", "500E6", etc.
    ...
    def configure_scope(self, ch: int):
        ...
        # Apply user's termination setting
        term_value = "50" if self.termination == "50" else "1E6"
        self.inst.scope_write(f"CH{ch}:TERmination {term_value}")
        self._log(f"CH{ch} termination: {'50Ω' if self.termination == '50' else '1MΩ'}")
        
        # Apply user's bandwidth setting
        if self.bandwidth and self.bandwidth != "FULL":
            self.inst.scope_write(f"CH{ch}:BANdwidth {self.bandwidth}")
            bw_mhz = float(self.bandwidth) / 1e6
            self._log(f"CH{ch} bandwidth limited to {bw_mhz:.0f} MHz")
        else:
            self.inst.scope_write(f"CH{ch}:BANdwidth FULL")
            self._log(f"CH{ch} bandwidth: FULL")
        
        # --- Current shunt scaling via SCALERATio ---
        # For shunt current measurement, use SCALERATio (NOT PROBEFunc:EXTAtten).
        # SCALERATio converts voltage to current on the display and in all measurements.
        # The scope reads voltage across the shunt, multiplies by the ratio.
        # For 10Ω shunt: I = V/10, so ratio = 0.1  (measurement returns actual amps)
        # For 0.5Ω shunt: I = V/0.5, so ratio = 2.0
        # Works on both MSO2 and MSO4/5/6. No additional math needed on readout.
        scale_ratio = 1.0 / self.SHUNT_RESISTANCE
        self._log(f"Setting CH{ch}:SCALERATio {scale_ratio} (1/{self.SHUNT_RESISTANCE}Ω shunt)")
        # EXTUNITS:STATE must be ON before SCALERATio takes effect
        self.inst.scope_write(f"CH{ch}:PROBEFunc:EXTUnits:STATE 1")
        self.inst.scope_write(f"CH{ch}:SCALERATio {scale_ratio}")
    ...
    def measure_currents(self, ch: int) -> Tuple[float, float]:
        ...
        Uses run/wait/stop acquisition for DC steady-state measurements:
        1. Put scope in RUN mode (free-running with AUTO trigger)
        2. Wait 0.5s for acquisitions to accumulate
        3. Verify at least one acquisition completed
        4. Stop acquisition
        5. Read MEAN measurement from captured data
        
        This is the correct approach for stable DC signals (shunt current, 
        power supply output). Single-sequence mode is better for triggered 
        events like edges, delay, rise/fall time, and serial data.
        ...
        # Measure from scope using run/wait/stop (NOT single sequence)
        # SCALERATio already converts voltage to amps — readout is actual amps
        scope_current = 0.0
        try:
            # Step 1: Clear stale measurement data (important after SMU voltage change)
            self.inst.scope_write("CLEAR")
            
            # Step 2: Start free-running acquisition with AUTO trigger
            self.inst.scope_write("ACQuire:STOPAfter RUNSTOP")
            self.inst.scope_write("ACQuire:STATE RUN")
            
            # Step 3: Wait for data to accumulate (2s for reliable capture at 20ms/div)
            time.sleep(2.0)
            
            # Step 4: Verify at least one acquisition completed
            try:
                acq_count = self.inst.scope_query("ACQuire:NUMACq?")
                if acq_count:
                    count = int(acq_count.strip())
                    if count < 1:
                        self._log("WARNING: No acquisitions yet, waiting longer...")
                        time.sleep(1.0)
                    else:
                        self._log(f"Acquisitions captured: {count}")
            except Exception:
                pass  # If query fails, the 2s wait should be enough
            
            # Step 5: Stop acquisition
            self.inst.scope_write("ACQuire:STATE STOP")
            time.sleep(0.1)  # Brief settle after stop
            
            # Step 6: Read mean current (already in amps via SCALERATio)
            result = self.inst.scope_query("MEASUrement:MEAS1:RESUlts:CURRentacq:MEAN?")
            if result:
                current_amps = float(result)
                if current_amps < 9e36:  # Check for 9.9E37 (no valid data)
                    scope_current = current_amps * 1000  # Convert A to mA
                    self._log(f"Scope measured: {scope_current:.3f} mA")
                else:
                    self._log("WARNING: Scope returned invalid measurement (9.9E37)")
        except Exception as e:
    ...
    def measure_scope_current(self, ch: int, expected_current_mA: float) -> float:
        ...
        If clipping is detected, auto-scales up and re-measures until clean.
        
        Args:
            ch: Scope channel number  
            expected_current_mA: Expected current from SMU reading (for trigger level)
        """
        MAX_RESCALE_ATTEMPTS = 6  # Safety limit
        
        for attempt in range(MAX_RESCALE_ATTEMPTS + 1):
            try:
                ...
                # Clear stale measurement data before new acquisition
                self.inst.scope_write("CLEAR")
                
                # Run mode - let it acquire for a bit then stop
                self.inst.scope_write("ACQuire:STOPAfter RUNSTop")
                self.inst.scope_write("ACQuire:STATE RUN")
                time.sleep(2.0)  # MSO2 needs ~2s at 20ms/div
                self.inst.scope_write("ACQuire:STATE STOP")
                time.sleep(0.2)
                ...
                if clipping and clipping.strip() == "1":
                    if attempt >= MAX_RESCALE_ATTEMPTS:
                        self._log(f"WARNING: CH{ch} still clipping after {MAX_RESCALE_ATTEMPTS} rescale attempts")
                        break  # Give up, return whatever we can read
                    
                    # Read current scale and step up
                    cur_scale_str = self.inst.scope_query(f"CH{ch}:SCAle?")
                    try:
                        cur_scale = float(cur_scale_str.strip())
                    except (ValueError, AttributeError):
                        cur_scale = 0.005
                    
                    # Jump to next 1-2-5 step
                    new_scale = self._next_125_scale(cur_scale)
                    self._log(f"Clipping at {self._format_scale(cur_scale)}/div — rescaling to {self._format_scale(new_scale)}/div")
                    self.inst.scope_write(f"CH{ch}:SCAle {new_scale}")
                    
                    # Also re-center offset on expected value
                    self.inst.scope_write(f"CH{ch}:OFFSet {trigger_level_A}")
                    continue  # Re-acquire with new scale
                
                # No clipping — read the measurement
                result = self.inst.scope_query("MEASUrement:MEAS1:RESUlts:CURRentacq:MEAN?")
                if result:
        ...
        # Fallback: try to read whatever is there
        try:
            result = self.inst.scope_query("MEASUrement:MEAS1:RESUlts:CURRentacq:MEAN?")
            if result:
                current_mA = float(result) * 1000
                self._log(f"Scope measured (with clipping): {current_mA:.3f} mA")
                return current_mA
        except Exception:
            pass
        return 0.0
    ...
    @staticmethod
    def _next_125_scale(current_scale: float) -> float:
        """Return the next scale up in a 1-2-5 sequence."""
        # Full 1-2-5 sequence from 100µA to 500mA
        steps = [
        ...
        for s in steps:
            if s > current_scale * 1.01:  # 1% tolerance for float comparison
                return s
        return current_scale * 2  # Beyond table, just double
    ...
    def setup_vertical_for_current(self, ch: int, expected_current_mA: float):
        ...
        Keeps increasing scale until there is NO clipping — never gives up.
        ...
        # 1-2-5 sequence covering 100µA/div to 500mA/div
        scales = [
            0.0001, 0.0002, 0.0005,   # 100µA, 200µA, 500µA
            0.001,  0.002,  0.005,     # 1mA, 2mA, 5mA
            0.010,  0.020,  0.050,     # 10mA, 20mA, 50mA
            0.100,  0.200,  0.500,     # 100mA, 200mA, 500mA
        ]
        
        # Pick a starting index based on expected current (skip obviously-too-small scales)
        # Want at least 2 divisions of headroom: scale > expected / 4
        start_idx = 0
        target_min = abs(current_A) / 4.0
        ...
        resolved = False
        for scale in scales[start_idx:]:
            self._log(f"Trying vertical: {self._format_scale(scale)}/div, offset: {current_A*1000:.3f} mA")
            
            self.inst.scope_write(f"CH{ch}:SCAle {scale}")
            # Center the expected signal on screen
            ...
            clipping = self.inst.scope_query(f"CH{ch}:CLIPping?")
            if clipping and clipping.strip() == "1":
                self._log(f"Clipping at {self._format_scale(scale)}/div — increasing scale...")
                continue
            else:
                self._log(f"Vertical setup complete: {self._format_scale(scale)}/div, no clipping")
                resolved = True
                break
        
        if not resolved:
            self._log("WARNING: Clipping persists at maximum scale — signal may exceed measurement range")
    ...
    @staticmethod
    def _format_scale(scale_A: float) -> str:
        """Format a scale value in appropriate units (µA, mA, or A)"""
        if scale_A < 0.001:
            return f"{scale_A*1e6:.0f} µA"
        elif scale_A < 1.0:
            return f"{scale_A*1e3:.0f} mA"
        else:
            return f"{scale_A:.1f} A"

    def take_preliminary_measurement(self, ch: int) -> float:
        """Take a quick measurement to determine actual current level for scaling.
        Auto-scales up if clipping is detected to get an accurate reading."""
        MAX_ATTEMPTS = 6
        
        for attempt in range(MAX_ATTEMPTS):
            # Clear stale data, run/wait/stop for DC steady-state
            self.inst.scope_write("CLEAR")
            self.inst.scope_write("ACQuire:STOPAfter RUNSTOP")
            self.inst.scope_write("ACQuire:STATE RUN")
            time.sleep(2.0)  # MSO2 needs ~2s at 20ms/div
            ...
            # Check for clipping before trusting the measurement
            clipping = self.inst.scope_query(f"CH{ch}:CLIPping?")
            if clipping and clipping.strip() == "1":
                # Read current scale and step up
                cur_scale_str = self.inst.scope_query(f"CH{ch}:SCAle?")
                try:
                    ...
                    cur_scale = 0.001
                new_scale = self._next_125_scale(cur_scale)
                self._log(f"Preliminary: clipping at {self._format_scale(cur_scale)}/div — rescaling to {self._format_scale(new_scale)}/div")
                self.inst.scope_write(f"CH{ch}:SCAle {new_scale}")
                continue
            ...
            if result:
                current_A = float(result)
                if current_A < 9e36:  # Check for invalid
                    current_mA = current_A * 1000
                    self._log(f"Preliminary measurement: {current_mA:.3f} mA")
                    return current_mA
            break  # No clipping but measurement failed — don't loop
        ...
        try:
            ...
            for scope_path in scope_paths:
                try:
                    ...
                    self._log(f"Screenshot path {scope_path} failed: {e}")
                    continue
...
class TektronixProductionTestApp:
    def __init__(self):
        ...
        self._suite_queue = []  # v1.1.1: Multi-suite run queue
        ...
        # v1.1.1: Favorites manager for suite preferences
        self.favorites_mgr = FavoritesManager() if V111_AVAILABLE else None
        # v1.1.1: Help docs directory
        self._docs_dir = self._find_docs_dir()
    ...
    def _create_header(self):
        ...
        # v1.1.1: Help button
        self.help_btn = RoundedButton(btn_frame, "❓ Help",
                                      command=self._show_help_menu, style="secondary")
    ...
    def _create_instrument_card(self, parent, inst: InstrumentInfo):
        ...
        if inst.instrument_type == "Oscilloscope":
            if inst.is_connected:
                btn = RoundedButton(inner, "Disconnect",
                                   command=lambda i=inst: self._disconnect_scope(i),
                                   style="secondary", width=85, height=26)
            else:
            ...
            if inst.is_connected:
                btn = RoundedButton(inner, "Disconnect",
                                   command=lambda i=inst: self._disconnect_smu(i),
                                   style="secondary", width=85, height=26)
            else:
                btn = RoundedButton(inner, "Connect",
            ...
            if inst.is_connected:
                btn = RoundedButton(inner, "Disconnect",
                                   command=lambda i=inst: self._disconnect_awg(i),
                                   style="secondary", width=85, height=26)
            else:
                btn = RoundedButton(inner, "Connect",
...
For educational and demonstration purposes."""
    ...
    def _find_docs_dir(self) -> str:
        """Find the docs directory containing help files."""
        script_dir = Path(__file__).parent
        candidates = [
            script_dir / "docs",
            script_dir / ".." / "docs",
            script_dir / ".." / "docs" / "local_docs",
            script_dir / ".." / "docs" / "reference",
        ]
        for path in candidates:
            if path.is_dir():
                return str(path.resolve())
        return str(script_dir / "docs")

    def _show_help_menu(self):
        """Show help menu as a popup near the Help button."""
        menu = tk.Menu(self.root, tearoff=0, bg=TekColors.BG_MEDIUM,
                      fg=TekColors.TEXT_PRIMARY, activebackground=TekColors.TEK_BLUE,
                      activeforeground="white", font=("Segoe UI", 10))
        
        menu.add_command(label="📄 Plugin Development Guide",
                        command=lambda: self._open_help_doc(
                            "TEK_PTA_PLUGIN_GUIDE.md", "Plugin Development Guide"))
        menu.add_command(label="📄 API Reference",
                        command=lambda: self._open_help_doc(
                            "TEK_PTA_API_REFERENCE.md", "Plugin API Reference"))
        menu.add_command(label="📄 Automation Guide",
                        command=lambda: self._open_help_doc(
                            "TEK_PTA_AUTOMATION_GUIDE.md", "Automation Development Guide"))
        menu.add_separator()
        menu.add_command(label="📋 New Plugin Template",
                        command=self._show_plugin_template)
        menu.add_separator()
        menu.add_command(label="🌐 Tek Automate (Online)",
                        command=lambda: webbrowser.open("https://abnasim.github.io/TekAutomate/"))
    ...
    def _open_help_doc(self, filename: str, title: str):
        """Open a markdown/text help document in a read-only viewer window."""
        # Search for the file in multiple locations
        search_paths = []
        if self._docs_dir:
            search_paths.append(os.path.join(self._docs_dir, filename))
        
        script_dir = Path(__file__).parent
        search_paths.extend([
            str(script_dir / filename),
            str(script_dir / "docs" / filename),
            str(script_dir / ".." / "docs" / filename),
            str(script_dir / ".." / "docs" / "local_docs" / filename),
            str(script_dir / ".." / "docs" / "reference" / filename),
        ])
        ...
        if content is None:
            TekStyledDialog.showwarning(
                ...
                f"Place help docs in one of these locations:\n"
        ...
        viewer.configure(bg=TekColors.BG_DARK)
        
        # Top bar with title and filename
        top = tk.Frame(viewer, bg=TekColors.BG_DARK)
        top.pack(fill=tk.X, padx=15, pady=(12, 5))
        
        tk.Label(top, text=f"📖 {title}", font=("Segoe UI", 14, "bold"),
                bg=TekColors.BG_DARK, fg=TekColors.TEK_CYAN).pack(side=tk.LEFT)
        
        if found_path:
            tk.Label(top, text=os.path.basename(found_path),
                    font=("Segoe UI", 9), bg=TekColors.BG_DARK,
                    fg=TekColors.TEXT_DIM).pack(side=tk.RIGHT)
        
        # Text content with scrollbar
        text_frame = tk.Frame(viewer, bg=TekColors.BG_DARK)
        text_frame.pack(fill=tk.BOTH, expand=True, padx=15, pady=(0, 10))
        
        text = tk.Text(
            text_frame, wrap="word", font=("Consolas", 10),
            bg="#0D1B2A", fg=TekColors.TEXT_PRIMARY, relief="flat",
            padx=15, pady=10, insertbackground=TekColors.TEXT_PRIMARY,
            selectbackground=TekColors.BG_LIGHT, spacing1=2, spacing3=2
        )
        ...
        text.tag_configure("h1", font=("Segoe UI", 16, "bold"), foreground=TekColors.TEK_CYAN,
                          spacing1=10, spacing3=6)
        text.tag_configure("h2", font=("Segoe UI", 13, "bold"), foreground="#4FC3F7",
                          spacing1=8, spacing3=4)
        text.tag_configure("h3", font=("Segoe UI", 11, "bold"), foreground="#81D4FA",
                          spacing1=6, spacing3=3)
        text.tag_configure("code_block", font=("Consolas", 10), background="#162030",
                          foreground="#98FB98", lmargin1=20, lmargin2=20)
        text.tag_configure("bullet", foreground=TekColors.TEK_CYAN)
        text.tag_configure("checkbox", foreground="#FFD700")
        text.tag_configure("bold_text", font=("Consolas", 10, "bold"))
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith("```"):
                ...
                text.tag_add("code_block", f"{line_num}.0", f"{line_num}.end")
            elif in_code_block:
                text.tag_add("code_block", f"{line_num}.0", f"{line_num}.end")
            elif stripped.startswith("# ") and not stripped.startswith("##"):
                text.tag_add("h1", f"{line_num}.0", f"{line_num}.end")
            elif stripped.startswith("## ") and not stripped.startswith("###"):
                text.tag_add("h2", f"{line_num}.0", f"{line_num}.end")
            elif stripped.startswith("### "):
                text.tag_add("h3", f"{line_num}.0", f"{line_num}.end")
            elif stripped.startswith("- [ ]") or stripped.startswith("- [x]"):
                text.tag_add("checkbox", f"{line_num}.0", f"{line_num}.5")
            elif stripped.startswith("- ") or stripped.startswith("* "):
                text.tag_add("bullet", f"{line_num}.0", f"{line_num}.2")
        
        # Make read-only
        text.configure(state="disabled")
        
        # Bottom bar with close button
        btn_frame = tk.Frame(viewer, bg=TekColors.BG_DARK)
        btn_frame.pack(fill=tk.X, padx=15, pady=(0, 12))
        
        RoundedButton(btn_frame, "Close", command=viewer.destroy,
                     style="secondary", width=100, height=30).pack(side=tk.RIGHT)
        
        # Center on parent
        viewer.update_idletasks()
        x = self.root.winfo_x() + (self.root.winfo_width() - 900) // 2
        y = self.root.winfo_y() + (self.root.winfo_height() - 700) // 2
        viewer.geometry(f"+{max(0, x)}+{max(0, y)}")

    def _show_plugin_template(self):
        """Show the plugin starter template with option to save a copy."""
        try:
            from tek_pta_v111_upgrade import _get_plugin_template
...
Place this file in the test_suites/ folder and restart Tek PTA.
See TEK_PTA_PLUGIN_GUIDE.md for full documentation.
"""
# See the Plugin Development Guide for the complete template.
# Help → Plugin Development Guide
'''
        
        viewer = tk.Toplevel(self.root)
        viewer.title("Help — New Plugin Template")
        ...
        tk.Label(top, text="📋 New Plugin Template", font=("Segoe UI", 14, "bold"),
                bg=TekColors.BG_DARK, fg=TekColors.TEK_CYAN).pack(side=tk.LEFT)
        
        # Save button
        def save_template():
            path = filedialog.asksaveasfilename(
                parent=viewer, title="Save Plugin Template",
                defaultextension=".py",
                filetypes=[("Python files", "*.py")],
                initialfile="my_test_suite.py",
                initialdir=str(Path(__file__).parent / "test_suites")
            )
            if path:
                with open(path, 'w', encoding='utf-8') as f:
                    f.write(template)
                TekStyledDialog.showinfo(viewer, "Saved",
                    f"Template saved to:\n{path}")
        
        RoundedButton(top, "💾 Save Copy...", command=save_template,
                     style="primary", width=120, height=30).pack(side=tk.RIGHT)
        ...
        text = tk.Text(text_frame, wrap="none", font=("Consolas", 10),
                      bg="#0D1B2A", fg="#98FB98", relief="flat", padx=15, pady=10)
...
• Configuration is saved per test suite"""
    ...
    def _create_channel_config(self, parent):
        """Create channel configuration with probe settings and REF source support"""
        ch_frame = ttk.LabelFrame(parent, text="Scope Input Source")
        ch_frame.pack(fill=tk.X, padx=15, pady=10)
        
        row1 = ttk.Frame(ch_frame)
        row1.pack(fill=tk.X, padx=10, pady=5)
        ttk.Label(row1, text="Measurement Source:").pack(side=tk.LEFT)
        self.scope_channel = tk.StringVar(value="CH1")
        # v1.1.1: Include CH1-CH8 + separator + REF1-REF4
        source_values = ["CH1", "CH2", "CH3", "CH4", "CH5", "CH6", "CH7", "CH8",
                         "───────", "REF1", "REF2", "REF3", "REF4"]
        ch_combo = ttk.Combobox(row1, textvariable=self.scope_channel,
                               values=source_values, width=8)
        ch_combo.pack(side=tk.LEFT, padx=10)
        
        # Prevent selecting the separator
        def on_channel_select(event):
            val = self.scope_channel.get()
            if val.startswith("──"):
                self.scope_channel.set("CH1")
        ch_combo.bind("<<ComboboxSelected>>", on_channel_select)
        
        # Hint label
        self._source_hint = tk.Label(row1, text="", font=("Segoe UI", 8, "italic"),
                                    bg=TekColors.BG_DARK, fg=TekColors.TEK_CYAN)
        self._source_hint.pack(side=tk.LEFT, padx=10)
        
        def update_source_hint(*args):
            val = self.scope_channel.get()
            if val.startswith("REF"):
                self._source_hint.configure(text="Using reference waveform — load via References tab")
            else:
                self._source_hint.configure(text="")
        self.scope_channel.trace_add("write", update_source_hint)
        
        probe_frame = ttk.LabelFrame(parent, text="Probe / Input Settings")
        probe_frame.pack(fill=tk.X, padx=15, pady=10)
        
        # Row 1: Probe type dropdown (auto-sets termination + bandwidth)
        row_probe = ttk.Frame(probe_frame)
        row_probe.pack(fill=tk.X, padx=10, pady=5)
        ttk.Label(row_probe, text="Probe Type:").pack(side=tk.LEFT)
        self.ch_probe_type = tk.StringVar(value="BNC Direct (50Ω)")
        probe_combo = ttk.Combobox(row_probe, textvariable=self.ch_probe_type,
                                   values=[
                                       "BNC Direct (50Ω)",
                                       "BNC Direct (1MΩ)",
                                       "TPP0500B (500MHz 10x)",
        ...
        probe_combo.pack(side=tk.LEFT, padx=10)
        
        # Probe preset mapping: probe_type -> (termination, bandwidth, attenuation)
        self._probe_presets = {
            "BNC Direct (50Ω)":      ("50",  "FULL", "1x"),
            "BNC Direct (1MΩ)":      ("1M",  "FULL", "1x"),
            "TPP0500B (500MHz 10x)": ("1M",  "500E6", "10x"),
            "TPP1000 (1GHz 10x)":    ("1M",  "FULL",  "10x"),
            "TCP0030A (Current)":    ("1M",  "FULL",  "1x"),
            "TIVH08 (HV Diff)":      ("50",  "FULL",  "10x"),
            "TICS Shunt Board":      ("1M",  "20E6",  "1x"),
            "Custom":                (None, None, None),  # Don't change
        }
        
        def _on_probe_select(event=None):
            probe = self.ch_probe_type.get()
            preset = self._probe_presets.get(probe)
            if preset and preset[0] is not None:
                term, bw, atten = preset
                self.ch_termination.set(term)
                self.ch_bandwidth.set(bw)
                self.ch_attenuation.set(atten)
        
        probe_combo.bind("<<ComboboxSelected>>", _on_probe_select)
        
        # Row 2: Termination + Attenuation
        row2 = ttk.Frame(probe_frame)
        row2.pack(fill=tk.X, padx=10, pady=5)
        ttk.Label(row2, text="Termination:").pack(side=tk.LEFT)
        self.ch_termination = tk.StringVar(value="1M")
        ttk.Radiobutton(row2, text="50Ω", variable=self.ch_termination, value="50").pack(side=tk.LEFT, padx=10)
        ttk.Radiobutton(row2, text="1MΩ", variable=self.ch_termination, value="1M").pack(side=tk.LEFT, padx=10)
        
        ttk.Label(row2, text="    Attenuation:").pack(side=tk.LEFT)
        self.ch_attenuation = tk.StringVar(value="1x")
        atten_combo = ttk.Combobox(row2, textvariable=self.ch_attenuation,
                                   values=["1x", "10x", "100x", "1000x"], width=6, state="readonly")
        atten_combo.pack(side=tk.LEFT, padx=10)
    ...
    def _restore_defaults(self):
        ...
        if self.current_suite:
            ...
            if self.current_suite.test_type == "afg_freq":
                ...
                self.scope_channel.set(f"CH{cfg.get('channel', 3)}")
    ...
    def _show_config_for_test_type(self, test_type: str):
        ...
        if test_type == "afg_freq":
            ...
            self.scope_channel.set("CH2")
            self.ch_probe_type.set("BNC Direct (1MΩ)")
            self.ch_termination.set("1M")
            self.ch_bandwidth.set("FULL")
            self.ch_attenuation.set("1x")
            ...
            self.scope_channel.set("CH1")  # Primary channel (trigger source)
            self.ch_probe_type.set("BNC Direct (50Ω)")
            self.ch_termination.set("50")
            self.ch_bandwidth.set("FULL")
            self.ch_attenuation.set("1x")
    ...
    def _parse_channel_value(self) -> tuple:
        """Parse scope_channel dropdown value.
        Returns (channel_number: int, is_ref: bool, label: str)
        ...
        val = self.scope_channel.get()
        if val.startswith("REF"):
            return int(val[3:]), True, val
        elif val.startswith("CH"):
            return int(val[2:]), False, val
        else:
            # Fallback for raw numbers (shouldn't happen with new UI)
            return int(val), False, f"CH{val}"
    ...
    def _setup_callbacks(self):
        def _on_complete_with_pause(tp):
            ...
            IMPORTANT: clear() must happen BEFORE put() to prevent a race condition.
            If put() comes first, the UI thread can process the message and call set()
            before we call clear(), wiping out the set() and causing a deadlock.
            """
            if tp.status == TestStatus.FAIL:
                # Clear FIRST so UI's set() won't be overwritten
            ...
            if tp.status == TestStatus.FAIL:
                ...
                if not self.failure_pause_event.wait(timeout=300):
                    # Timed out — force resume so engine doesn't hang forever
                    self.msg_queue.put(('log', "WARNING: Failure pause timed out after 5 min, resuming"))
        ...
        try:
            while True:
                ...
                if msg_type == 'log':
                    ...
                    self._on_done(*data)
                elif msg_type == 'scope_connect':
                    self._on_scope_connected(*data)
                elif msg_type == 'smu_connect':
                    self._on_smu_connected(*data)
                elif msg_type == 'awg_connect':
                    self._on_awg_connected(*data)
        except queue.Empty:
            pass
    ...
    def _connect_scope(self, inst: InstrumentInfo):
        ...
        def _do_connect():
            ...
            # Post result back to UI thread via queue
            self.msg_queue.put(('scope_connect', (ok, msg, inst)))
        
        threading.Thread(target=_do_connect, daemon=True).start()

    def _on_scope_connected(self, ok, msg, inst):
        """Handle scope connection result on UI thread"""
        if ok:
            self._log(msg)
            inst.is_connected = True
            # Log probe settings from UI (no auto-detect needed)
            probe = self.ch_probe_type.get() if hasattr(self, 'ch_probe_type') else "Unknown"
            term = self.ch_termination.get() if hasattr(self, 'ch_termination') else "?"
            self._log(f"Probe: {probe}, Termination: {term}")
    ...
    def _connect_smu(self, inst: InstrumentInfo):
        ...
        def _do_connect():
            ...
            self.msg_queue.put(('smu_connect', (ok, msg, inst)))
        
        threading.Thread(target=_do_connect, daemon=True).start()

    def _on_smu_connected(self, ok, msg, inst):
        """Handle SMU connection result on UI thread"""
        if ok:
            self._log(msg)
            inst.is_connected = True
            self._refresh_instruments()
    ...
    def _connect_awg(self, inst: InstrumentInfo):
        ...
        def _do_connect():
            ...
            self.msg_queue.put(('awg_connect', (ok, msg, inst)))
        
        threading.Thread(target=_do_connect, daemon=True).start()

    def _on_awg_connected(self, ok, msg, inst):
        """Handle AWG connection result on UI thread"""
        if ok:
            self._log(msg)
            inst.is_connected = True
            self._refresh_instruments()
    ...
    def _disconnect_scope(self, inst: InstrumentInfo):
        """Disconnect oscilloscope without restarting application."""
        self._log(f"Disconnecting {inst.model}...")
        ok, msg = self.inst.disconnect_scope()
        if ok:
            self._log(f"✓ {msg}")
    ...
    def _disconnect_smu(self, inst: InstrumentInfo):
        """Disconnect SMU — turns off output first for safety."""
        self._log(f"Disconnecting {inst.model}...")
        # Safety: turn off SMU output before disconnecting
        try:
            self.inst.smu_write("smu.source.output = smu.OFF")
            self._log("SMU output disabled before disconnect")
        except Exception:
            pass  # SMU may already be unresponsive
        ok, msg = self.inst.disconnect_smu()
        if ok:
            self._log(f"✓ {msg}")
            inst.is_connected = False
            self._refresh_instruments()
    ...
    def _disconnect_awg(self, inst: InstrumentInfo):
        """Disconnect AWG/AFG without restarting application."""
        self._log(f"Disconnecting {inst.model}...")
        ok, msg = self.inst.disconnect_awg()
        if ok:
            self._log(f"✓ {msg}")
            inst.is_connected = False
            self._refresh_instruments()
    ...
    def _show_suite_selector(self):
        ...
        dialog.title("Select Test Suites")
        dialog.geometry("1250x700")
        ...
        sort_frame.pack(fill=tk.X, padx=20, pady=(0, 8))
        
        tk.Label(sort_frame, text="Sort by:", font=TekFonts.SMALL,
                bg=TekColors.BG_DARK, fg=TekColors.TEXT_SECONDARY).pack(side=tk.LEFT)
        
        sort_var = tk.StringVar(value="favorites")
        
        def apply_sort():
            nonlocal sorted_suites
            sort_type = sort_var.get()
            if sort_type == "favorites" and self.favorites_mgr:
                sorted_suites = sorted(self.test_suites, key=lambda s: (
                    0 if self.favorites_mgr.is_favorite(s.name) else 1,
                    s.name.lower()
                ))
            elif sort_type == "last_used" and self.favorites_mgr:
                sorted_suites = sorted(self.test_suites, key=lambda s: (
                ...
                sorted_suites = sorted(self.test_suites, key=lambda s: s.created_time or "", reverse=True)
            elif sort_type == "modified":
                sorted_suites = sorted(self.test_suites, key=lambda s: s.modified_time or "", reverse=True)
            else:
                sorted_suites = list(self.test_suites)
            rebuild_list()
        
        for text, val in [("★ Favorites", "favorites"), ("🕐 Last Used", "last_used"),
                          ("A-Z", "name"), ("📅 Created", "created"), ("📅 Modified", "modified"),
                          ("Default", "default")]:
            ttk.Radiobutton(sort_frame, text=text, variable=sort_var, value=val,
                           command=apply_sort).pack(side=tk.LEFT, padx=(8, 3))
        
        # Select All / None buttons
        sel_frame = tk.Frame(sort_frame, bg=TekColors.BG_DARK)
        ...
        RoundedButton(sel_frame, "Select All", command=select_all,
                     style="secondary", width=80, height=26).pack(side=tk.LEFT, padx=2)
        RoundedButton(sel_frame, "None", command=select_none,
                     style="secondary", width=60, height=26).pack(side=tk.LEFT, padx=2)
        ...
        check_vars = {}  # suite_name -> BooleanVar
        sorted_suites = list(self.test_suites)
        
        # Initial sort by favorites if available
        ...
        def rebuild_list():
            ...
            # Column headers
            hdr = tk.Frame(scroll_frame, bg=TekColors.BG_LIGHT)
            hdr.pack(fill=tk.X, pady=(0, 4), padx=5)
            hdr_inner = tk.Frame(hdr, bg=TekColors.BG_LIGHT)
            hdr_inner.pack(fill=tk.X, padx=8, pady=4)
            
            tk.Label(hdr_inner, text="", width=3, bg=TekColors.BG_LIGHT,
                    fg=TekColors.TEXT_DIM).pack(side=tk.LEFT)  # star column
            tk.Label(hdr_inner, text="", width=3, bg=TekColors.BG_LIGHT).pack(side=tk.LEFT)  # checkbox
            tk.Label(hdr_inner, text="Suite Name", font=("Segoe UI", 9, "bold"),
                    bg=TekColors.BG_LIGHT, fg=TekColors.TEXT_SECONDARY,
                    width=28, anchor=tk.W).pack(side=tk.LEFT, padx=(5, 10))
            tk.Label(hdr_inner, text="Description", font=("Segoe UI", 9, "bold"),
                    bg=TekColors.BG_LIGHT, fg=TekColors.TEXT_SECONDARY,
                    anchor=tk.W).pack(side=tk.LEFT, fill=tk.X, expand=True)
            tk.Label(hdr_inner, text="Equipment", font=("Segoe UI", 9, "bold"),
                    bg=TekColors.BG_LIGHT, fg=TekColors.TEXT_SECONDARY,
                    width=14, anchor=tk.W).pack(side=tk.RIGHT)
            tk.Label(hdr_inner, text="Modified", font=("Segoe UI", 9, "bold"),
                    bg=TekColors.BG_LIGHT, fg=TekColors.TEXT_SECONDARY,
                    width=12, anchor=tk.W).pack(side=tk.RIGHT, padx=(0, 5))
            tk.Label(hdr_inner, text="Created", font=("Segoe UI", 9, "bold"),
                    bg=TekColors.BG_LIGHT, fg=TekColors.TEXT_SECONDARY,
                    width=12, anchor=tk.W).pack(side=tk.RIGHT, padx=(0, 5))
            ...
            for i, suite in enumerate(sorted_suites):
                ...
                # Create BooleanVar if not exists
                if name not in check_vars:
                    check_vars[name] = tk.BooleanVar(value=False)
                
                row_bg = TekColors.BG_CARD if i % 2 == 0 else TekColors.BG_MEDIUM
                
                card = tk.Frame(scroll_frame, bg=row_bg)
                card.pack(fill=tk.X, pady=1, padx=5)
                
                inner = tk.Frame(card, bg=row_bg)
                inner.pack(fill=tk.X, padx=8, pady=6)
                
                # Favorite star
                is_fav = self.favorites_mgr.is_favorite(name) if self.favorites_mgr else False
                star_label = tk.Label(
                    inner, text="★" if is_fav else "☆",
                    font=("Segoe UI", 14),
                    fg="#FFD700" if is_fav else "#555555",
                    bg=row_bg, cursor="hand2", width=2
                )
                star_label.pack(side=tk.LEFT, padx=(0, 2))
                
                def toggle_fav(n=name, lbl=star_label):
                    if self.favorites_mgr:
                        is_f = self.favorites_mgr.toggle_favorite(n)
                        lbl.configure(
                            text="★" if is_f else "☆",
                            fg="#FFD700" if is_f else "#555555"
                        )
                star_label.bind("<Button-1>", lambda e, n=name, l=star_label: toggle_fav(n, l))
                
                # Checkbox (larger)
                cb = tk.Checkbutton(
                    inner, variable=check_vars[name],
                    bg=row_bg, activebackground=row_bg,
                    selectcolor=TekColors.BG_LIGHT,
                    font=("Segoe UI", 12)
                )
                ...
                # Suite name
                name_label = tk.Label(
                    inner, text=name, font=("Segoe UI", 10, "bold"),
                    bg=row_bg, fg=TekColors.TEXT_PRIMARY, anchor=tk.W, width=28
                )
                name_label.pack(side=tk.LEFT, padx=(0, 10))
                
                # Description (truncated)
                desc = suite.description.replace('\n', ' ').strip()
                if len(desc) > 60:
                    desc = desc[:57] + "..."
                desc_label = tk.Label(
                    inner, text=desc, font=("Segoe UI", 9),
                    bg=row_bg, fg=TekColors.TEXT_SECONDARY, anchor=tk.W
                )
                desc_label.pack(side=tk.LEFT, fill=tk.X, expand=True)
                
                # Required equipment (compact)
                if suite.required_instruments:
                    equip = ", ".join(suite.required_instruments)
                    if len(equip) > 16:
                        equip = equip[:13] + "..."
                    tk.Label(inner, text=equip, font=("Segoe UI", 8),
                            bg=row_bg, fg=TekColors.TEK_CYAN, width=14,
                            anchor=tk.W).pack(side=tk.RIGHT)
                else:
                    tk.Label(inner, text="", width=14, bg=row_bg).pack(side=tk.RIGHT)
                
                # Modified date
                mod_str = suite.modified_time or "—"
                tk.Label(inner, text=mod_str, font=("Segoe UI", 8),
                        bg=row_bg, fg=TekColors.TEXT_DIM, width=12,
                        anchor=tk.W).pack(side=tk.RIGHT, padx=(0, 5))
                
                # Created date
                cre_str = suite.created_time or "—"
                tk.Label(inner, text=cre_str, font=("Segoe UI", 8),
                        bg=row_bg, fg=TekColors.TEXT_DIM, width=12,
                        anchor=tk.W).pack(side=tk.RIGHT, padx=(0, 5))
                ...
                for widget in [card, inner, name_label, desc_label]:
                    widget.bind("<Button-1>",
                        lambda e, var=check_vars[name]: var.set(not var.get()))
        ...
        def select():
            ...
            # Get selected suites in order
            selected = [s for s in sorted_suites if check_vars.get(s.name, tk.BooleanVar()).get()]
            
            if not selected:
                TekStyledDialog.showwarning(dialog, "No Selection",
                    "Please select at least one test suite.")
                return
            
            # Save selection for next session
            if self.favorites_mgr:
                sel_names = [s.name for s in selected]
                self.favorites_mgr.save_last_selection(sel_names)
                for s in selected:
                    self.favorites_mgr.record_usage(s.name)
            
            dialog.destroy()
            
            if len(selected) == 1:
                # Single suite — use existing load logic
                self._load_suite(selected[0])
            else:
                # Multiple suites — store queue for sequential run
                self._suite_queue = selected
                self._load_suite(selected[0])  # Load first suite for config
                suite_names = "\n".join(f"  {i+1}. {s.name}" for i, s in enumerate(selected))
                self._log(f"Suite queue loaded ({len(selected)} suites):\n{suite_names}")
                self.suite_label.configure(
                    text=f"Queue: {len(selected)} suites — {selected[0].name} (first)")
    ...
    def _load_suite(self, suite: TestSuite):
        ...
        if suite.test_type == "afg_freq":
            ...
            self.specific_values.set(", ".join(str(v) for v in voltages))
            self.scope_channel.set(f"CH{cfg.get('channel', 3)}")
            # Load shunt resistance from suite config (default 10Ω)
            self.shunt_resistance.set(str(cfg.get("shunt_resistance", 10)))
            ...
            self.scope_channel.set(f"CH{cfg.get('channel', 2)}")
            ...
            self.scope_channel.set(f"CH{cfg.get('channel', 1)}")
            ...
            self.scope_channel.set(f"CH{cfg.get('channel_from', 1)}")
            ...
            def _plugin_on_complete(tp):
                if tp.status.value == "FAIL":
                    self.failure_pause_event.clear()  # Clear BEFORE put (race condition fix)
                self.msg_queue.put(('complete', tp))
                if tp.status.value == "FAIL":
                    if not self.failure_pause_event.wait(timeout=300):
                        self.msg_queue.put(('log', "WARNING: Failure pause timed out after 5 min, resuming"))
    ...
    def _show_setup_instructions(self) -> bool:
        ...
        if self.current_suite.test_type == "afg_freq":
            ...
            ch, is_ref, ch_label = self._parse_channel_value()
...
Connect AFG OUTPUT (rear) → {ch_label} (front) with 50Ω BNC cable
...
Probe: {ch_label} across the 10Ω shunt (tip on high side)
...
Connect antenna to {ch_label} (50Ω termination)
...
Output: Top 10 peaks sorted by amplitude"""
            ...
            instructions = f"Connect DUT → SMA cable → SMA-BNC adapter → {ch_label} (50Ω)  |  Rate: {data_rate} Gbps  |  Acq: {num_acq}"
    ...
    def _run_tests(self):
        ...
        # v1.1.1: Initialize queue tracking on first run
        is_queue_continuation = hasattr(self, '_queue_results') and self._queue_results
        if not is_queue_continuation:
            # First run — initialize queue state if there's a queue
            if self._suite_queue:
                self._queue_total = len(self._suite_queue) + 1  # +1 for current suite
                self._queue_results = []
                self._log(f"Starting suite queue ({self._queue_total} suites)")
                self.status_label.configure(
                    text=f"Queue: Suite 1/{self._queue_total} — {self.current_suite.name}",
                    fg=TekColors.TEK_CYAN
                )
        
        # Show setup instructions (skip for queue continuations — user already confirmed)
        if not is_queue_continuation:
            if not self._show_setup_instructions():
                # User cancelled — clear queue
                self._suite_queue = []
                if hasattr(self, '_queue_results'):
                    del self._queue_results
                if hasattr(self, '_queue_total'):
                    del self._queue_total
                return
        ...
        # Get channel from UI - supports CH1-CH8 and REF1-REF4
        ch, is_ref, ch_label = self._parse_channel_value()
        if is_ref:
            self._log(f"Source: {ch_label} (reference waveform)")
        ...
        if self.current_suite.test_type == "afg_freq":
            ...
            # Pass user's channel settings
            self.led_engine.termination = self.ch_termination.get()
            self.led_engine.bandwidth = self.ch_bandwidth.get()
    ...
    def _on_done(self, passed, failed):
        # Always clean up failure-pause state so nothing stays stuck
        self.failure_pause_event.set()
        self.test_paused_for_failure = False
        if self.failure_dialog is not None:
            try:
        ...
        try:
            ...
            # v1.1.1: Check if there are more suites in the queue
            if hasattr(self, '_suite_queue') and self._suite_queue:
                # Accumulate results for combined report
                if not hasattr(self, '_queue_results'):
                    self._queue_results = []
                ...
                # Pop next suite from queue
                next_suite = self._suite_queue.pop(0)
                queue_total = getattr(self, '_queue_total', 1)
                queue_idx = queue_total - len(self._suite_queue)
                
                self._log(f"\n{'='*50}")
                self._log(f"Suite queue: Starting {queue_idx} of {queue_total}: {next_suite.name}")
                self._log(f"{'='*50}")
                
                # Update status
                self.status_label.configure(
                    text=f"Queue: Suite {queue_idx}/{queue_total} — {next_suite.name}",
                    fg=TekColors.TEK_CYAN
                )
                
                # Load and run next suite
                self._load_suite(next_suite)
                # Small delay to let UI update before starting next run
                self.root.after(500, self._run_tests)
                return
            
            # Queue complete (or single suite done)
            ...
            # Check if this was a multi-suite run
            if hasattr(self, '_queue_results') and self._queue_results:
                # Add final suite results
                self._queue_results.append({
                    'suite': self.current_suite,
                    'passed': passed,
                    'failed': failed,
                ...
                total_p = sum(r['passed'] for r in self._queue_results)
                total_f = sum(r['failed'] for r in self._queue_results)
                n_suites = len(self._queue_results)
                
                summary_lines = []
                for r in self._queue_results:
                    status = "✓ PASS" if r['failed'] == 0 else f"✗ {r['failed']} FAIL"
                    summary_lines.append(f"  {r['suite'].name}: {status}")
                summary = "\n".join(summary_lines)
                
                self._log(f"\n{'='*50}")
                self._log(f"QUEUE COMPLETE — {n_suites} suites, {total_p} passed, {total_f} failed")
                self._log(summary)
                self._log(f"{'='*50}")
                
                if total_f == 0:
                    self.status_label.configure(text=f"✓ ALL {n_suites} SUITES PASSED", fg=TekColors.STATUS_PASS)
                    TekStyledDialog.showinfo(self.root, "Queue Complete",
                        f"All {n_suites} suites passed ({total_p} total tests)!\n\n{summary}")
                else:
                    self.status_label.configure(text=f"✗ {total_f} FAILURES across {n_suites} suites", fg=TekColors.STATUS_FAIL)
                    TekStyledDialog.showwarning(self.root, "Queue Complete",
                        f"{n_suites} suites: {total_p} passed, {total_f} failed\n\n{summary}")
            ...
            # Safety net: if anything in _on_done crashes, still re-enable UI
            self._log(f"Error during test completion: {e}")
            print(f"[Tek PTA] _on_done error: {e}")
            try:
                self.run_btn.configure(state=tk.NORMAL)
                self.stop_btn.configure(state=tk.DISABLED)
            except Exception:
    ...
    def _flush_message_queue(self):
        ...
        Must match the message types and argument handling in _process_messages."""
        while True:
            try:
                msg_type, data = self.msg_queue.get_nowait()
            except queue.Empty:
            ...
            try:
                if msg_type == 'log':
                    ...
                    self._add_result(data)
                    self._update_counters()
                    # Do NOT show failure dialog during flush — we're already wrapping up
                elif msg_type == 'progress':
                    self.progress.set(data[0])
                    self.status_label.configure(text=data[1])
                elif msg_type == 'screenshot':
                    self._show_screenshot(data)
                elif msg_type in ('scope_connect', 'smu_connect', 'awg_connect'):
                    pass  # Connection results during flush — ignore
                # Skip 'done' messages here — we're already in _on_done
            except Exception as e:
                # Don't let one bad message kill the entire flush
                print(f"[Tek PTA] Warning: error processing queued '{msg_type}' message: {e}")
    ...
    def _save_results(self):
        ...
        # Save text report immediately (fast, ~1ms)
        ...
        # Snapshot UI values for background thread (tkinter StringVars aren't thread-safe)
        operator_name = self.operator.get()
        dut_serial = self.dut_serial.get()
        output_dir = self.output_dir
        suite = self.current_suite
        probe_type = self.ch_probe_type.get() if hasattr(self, 'ch_probe_type') else "Unknown"
        
        # Generate plots and PDF in background so UI stays responsive
        def _generate_reports_background():
            # Thread-safe log helper (self._log touches tkinter directly)
            def bg_log(msg):
                self.msg_queue.put(('log', msg))
            
            try:
                if suite.test_type == "led_current":
                    self._save_iv_plot_impl(engine, output_dir, bg_log)
                elif suite.test_type == "afg_freq":
                    self._save_frequency_plot_impl(engine, output_dir, bg_log)
                
                self._save_pdf_report_impl(engine, suite, output_dir, 
                                           operator_name, dut_serial, bg_log, probe_type)
            except Exception as e:
                bg_log(f"Report generation error: {e}")
            finally:
                bg_log(f"Results saved to: {output_dir}")
        
        self._log("Generating PDF report (background)...")
        report_thread = threading.Thread(target=_generate_reports_background, daemon=True)
        report_thread.start()

    def _save_pdf_report_impl(self, engine, suite, output_dir, operator_name, dut_serial, log, probe_type="Unknown"):
        """Generate a professional PDF test report with logo, plots, and probe info.
        Thread-safe: does not access tkinter widgets. Uses log() callable for output."""
        if not REPORTLAB_AVAILABLE:
            log("PDF generation skipped (reportlab not installed)")
            return
        ...
        try:
            ...
            info_data = [
                ...
                ['Test Suite:', suite.name if suite else 'N/A'],
                ['Operator:', operator_name or 'N/A'],
                ['DUT Serial:', dut_serial or 'N/A'],
            ]
            if self.inst.scope_info:
            ...
            probe_data = [['Channel', 'Probe Type', 'Termination', 'Bandwidth']]
            
            # Use user-configured probe settings (reliable across all scope models)
            if engine and engine.test_points:
                # Get the measurement channel from test context
                term_label = "50Ω" if getattr(engine, 'termination', '1M') == "50" else "1MΩ"
                bw_val = getattr(engine, 'bandwidth', 'FULL')
                if bw_val and bw_val != "FULL":
                    try:
                        bw_label = f"{float(bw_val)/1e6:.0f} MHz"
                    except (ValueError, TypeError):
                        bw_label = bw_val
                else:
                    bw_label = "Full"
                probe_data.append(["Measurement Ch", probe_type, term_label, bw_label])
            
            # Also try auto-detect for additional connected probes (MSO4/5/6 only)
            if self.inst.scope:
                for ch in range(1, 9):
                    try:
                        probe_info = self.inst.get_probe_info(ch)
                        if probe_info.get("connected") and probe_info.get("type"):
                            probe_data.append([
                                f"CH{ch}",
                                probe_info["type"],
                                "",
            ...
            if len(probe_data) > 1:
                probe_table = Table(probe_data, colWidths=[1.2*inch, 2*inch, 1*inch, 1*inch])
                ...
                elements.append(Paragraph("No probe information available", normal_style))
            ...
            if engine and engine.test_points:
                ...
                elif suite.test_type in ("agc_sample", "awg70002b_pulse"):
                ...
                if suite.test_type == "eye_diagram":
            ...
            if suite.test_type == "led_current":
                plot_path = output_dir / "iv_characteristic.png"
                if plot_path.exists():
                ...
                plot_path = output_dir / "frequency_response.png"
                if plot_path.exists():
                    elements.append(PageBreak())
    ...
    def _save_iv_plot(self):
        """Main-thread wrapper for I-V plot generation"""
        self._save_iv_plot_impl(self.current_engine, self.output_dir, self._log)

    def _save_iv_plot_impl(self, engine, output_dir, log):
        """Generate I-V characteristic plot for LED current test (thread-safe)"""
        if not MATPLOTLIB_AVAILABLE:
            log("I-V plot skipped (matplotlib not installed)")
            return
        
        try:
            if not engine or not engine.test_points:
                log("I-V plot skipped (no test points)")
                return
            ...
            if not voltages or all(v == 0 for v in smu_currents):
                log("I-V plot skipped (no valid current data)")
                return
            
            log(f"Generating I-V plot with {len(voltages)} points...")
            ...
            log(f"I-V plot saved: {plot_path.name}")
            
        except Exception as e:
            log(f"I-V plot error: {e}")

    def _save_frequency_plot(self):
        """Main-thread wrapper for frequency plot generation"""
        self._save_frequency_plot_impl(self.current_engine, self.output_dir, self._log)

    def _save_frequency_plot_impl(self, engine, output_dir, log):
        """Generate frequency response plot for AFG test (thread-safe)"""
        if not MATPLOTLIB_AVAILABLE:
            log("Frequency plot skipped (matplotlib not installed)")
            return
        ...
        try:
            ...
            log(f"Frequency response plot saved: {plot_path.name}")
            
        except Exception as e:
            log(f"Frequency plot error: {e}")
...
def main():
    ...
    # v1.1.1: Show splash screen / disclaimer before starting
    if V111_AVAILABLE:
        splash_root = tk.Tk()
        splash_root.withdraw()
        if not SplashScreen.show(splash_root, version=__version__):
            splash_root.destroy()
            sys.exit(0)
        splash_root.destroy()
