...
try:
    from command_groups_mapping_DPO import COMMAND_TO_GROUP, COMMAND_GROUPS
    HAS_COMMAND_MAPPING = True
    print("Loaded DPO command group mapping.")
except ImportError:
    print("WARNING: 'command_groups_mapping_DPO.py' not found. Using pattern-based command detection.")
    COMMAND_TO_GROUP = {}
    COMMAND_GROUPS = {}
...
POSSIBLE_FILES = [
    os.path.join(PROJECT_ROOT, "MSO-DPO5000-B-DPO7000-C-DPO70000.docx"),
]
OUTPUT_FILENAME = os.path.join(PROJECT_ROOT, "public", "commands", "MSO_DPO_5k_7k_70K.json")
...
def is_arial_narrow(para):
    ...
    # If no runs or no font specified, check if it's a command pattern anyway
    # (some commands might not have explicit font but are still valid)
    return False

def is_lucida_console(para):
    """Check if paragraph uses Lucida Console font (for syntax)"""
    for run in para.runs:
        font_name = get_font_name(run)
...
def extract_commands(file_path):
    ...
    for i, para in enumerate(doc.paragraphs):
        ...
        if current_cmd:
            ...
            # Check for section headers (Arial Narrow Bold)
            if text.upper().startswith("RELATED COMMANDS"):
                first_word = "RELATED COMMANDS"
            elif first_word in SECTION_MAP and is_arial_narrow(para) and is_bold(para):
                ...
                if content_after and current_section == "syntax":
                    # Check if content_after is a syntax line (contains command-like pattern)
                    if is_scpi_command(content_after.split()[0]) if content_after.split() else False:
            ...
            # Check for syntax lines (Lucida Console font)
            if current_section == "syntax" and is_lucida_console(para):
...
def post_process(raw_commands):
    ...
    return {
        "version": "2.0", "manual": "MSO/DPO5000/B, DPO7000/C, DPO70000/B/C/D/DX/SX, DSA70000/B/C/D, and MSO70000/C/DX Series Programmer Manual",
        "groups": groups_dict,
