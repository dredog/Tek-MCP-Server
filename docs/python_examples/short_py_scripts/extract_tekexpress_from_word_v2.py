"""
TekExpress SCPI Command Extraction Script - Improved Version
...
def extract_command_header(text: str) -> Optional[str]:
    """Extract command header from text (e.g., 'TEKEXP:SELECT DEVICE' from 'TEKEXP:SELECT DEVICE,"<DeviceName>"')."""
    ...
    if match:
        ...
        # Extract command parts - include subcommand if present (e.g., DEVICE, SUITE, TEST)
        parts = cmd_part.split()
        if parts:
            header = parts[0].strip().replace('?', '')  # Base command
            # Include subcommand if present (next word that's not a quoted string or placeholder)
            if len(parts) > 1:
                ...
                if (next_part.isupper() and 
                    ...
                    not next_part.startswith('"') and
                    next_part not in ['<DeviceName>', '<SuiteName>', '<TestName>', '<Value>']):
...
def parse_argument_table(table) -> List[Dict]:
    ...
    # Check if it's a complex enumeration table (e.g., TestName | Value)
    elif len(header_row) >= 2 and any('testname' in h.lower() or ('name' in h.lower() and 'value' in ' '.join(header_row).lower()) for h in header_row):
        ...
        if values:
            arguments.append({
                "name": "testname" if 'testname' in header_row[0].lower() else "name",
                "type": "enumeration",
                "required": True,
                "position": 0,
                ...
                for row in table.rows[1:]:
                    ...
                    if len(cells) >= 2:
                        ...
                        if val and val.lower() not in ['table continued…', 'table continued', 'continued…', 'continued', '']:
                            # Handle "Included\nExcluded" format - split into separate values
                            if '\n' in val or 'Included' in val and 'Excluded' in val:
                                if 'Included' in val:
                                    value_options.add('Included')
                                if 'Excluded' in val:
                                    value_options.add('Excluded')
                            else:
...
def extract_commands(doc, group_name: str) -> List[Dict]:
    ...
    # Track description paragraphs (last 2 non-empty paragraphs before command)
    recent_paragraphs = []
    ...
    for i, para in enumerate(doc.paragraphs):
        ...
        if section_type:
            if current_cmd:
                ...
                current_section = section_type
            recent_paragraphs = []  # Reset on section header
            continue
        
        if not text:
            continue
        
        # Track recent paragraphs for description
        if not is_tekexp_command(text) and not section_type:
            recent_paragraphs.append(text)
            # Keep only last 3 paragraphs
        ...
        if is_tekexp_command(text) and ('(Set)' in text or '(Query)' in text):
            ...
            if header:
                # Check if we already have this command (to combine Set/Query)
                existing_cmd = None
                for cmd in commands:
                    if cmd["header"] == header:
                        existing_cmd = cmd
                        break
                
                if existing_cmd:
                    # Update existing command with additional syntax
                    if '(Set)' in text:
                        existing_cmd["syntax"]["set"] = text.replace('(Set)', '').strip()
                    if '(Query)' in text:
                        existing_cmd["syntax"]["query"] = text.replace('(Query)', '').strip()
                    current_cmd = existing_cmd
                else:
                    # Create new command
                    # Use recent paragraphs as description (filter to likely descriptions)
                    desc_parts = []
                    for p in recent_paragraphs:
                        p_lower = p.lower()
                        if (p_lower.startswith('set or query') or 
                            p_lower.startswith('this command') or
                            (20 < len(p) < 300 and not p.startswith('TEKEXP'))):
                            desc_parts.append(p)
    ...
    # Find "Command arguments" sections and match tables to commands
    cmd_para_indices = {cmd["para_index"]: cmd for cmd in commands}
    sorted_indices = sorted(cmd_para_indices.keys())
